...
interface ToleranceManagementProps {
    onEditTolerance: (tolerance: Tolerance) => void;
}

export const ToleranceManagement: React.FC<ToleranceManagementProps> = ({ onEditTolerance }) => {
    ...
    const processedTolerances = useMemo(() => {
        let items = showInactive ? [...mockTolerances] : mockTolerances.filter(t => t.status === 'Active');
                            ...
                            {processedTolerances.map(tol => {
                                ...
                                return (
                                    <tr key={tol.id} className="hover:bg-gray-700/40">
                                        ...
                                        {showInactive && (
                                            <td className="px-6 py-4">
                                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${tol.status === 'Active' ? 'bg-green-900 text-green-300' : 'bg-yellow-900 text-yellow-300'}`}>
                                                    {tol.status === 'Active' ? 'Activa' : 'Inactiva'}
                                                </span>
                                            </td>
                                        )}
                                        <td className="px-6 py-4 max-w-xs truncate" title={tol.notes}>{tol.notes}</td>
                                        <td className="px-6 py-4 text-right">
                                            <button onClick={() => onEditTolerance(tol)} className="font-medium text-sky-400 hover:text-sky-300">Editar</button>
                                        </td>
                                    </tr>
                                );
                            })}
